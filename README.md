# GE-IT-Skills-portfolio

<img width="1584" height="396" alt="Image" src="https://github.com/user-attachments/assets/24371e03-42d4-473f-9edd-affa34bd8e90" />

# Hi, I'm Anicca Chiam 👋

Welcome to my GitHub profile!

## About Me

🌱 I enjoy exploring new places, discovering new experiences, and learning something new every day.

🍳 I love cooking, hiking, going out, and finding inspiration from the world around me.

## Skills & Strengths

- 🗓️ Planning and organizing outings
- ⏳ Time management
- 🧭 Exploring and adapting to new environments
- ✨ Creating memorable experiences

## Interests

- 🍽️ Cooking & trying new recipes
- 🏞️ Hiking and outdoor adventures
- 🌍 Traveling and exploring places
- 💡 Discovering new ideas and experiences


---
<br>

<img width="640" height="400" alt="image" src="https://github.com/user-attachments/assets/ab8629a5-d657-4d55-b703-2cccd40f1fd0" />

<img width="640" height="400" alt="image" src="https://github.com/user-attachments/assets/c2c406aa-461e-41a1-8a34-10a6cf14caf8" />


# ✨ Sussy But Classy ✨

## 🌎 Wear the Change, Support the Future

### About the Brand

Sussy But Classy is a sustainable fashion brand that blends creativity, style, and responsibility through mindful and eco-conscious fashion. We believe fashion should express individuality while creating a positive impact on the environment.

**Brand Tagline:**  
Sussy but Classy, Thoughful Style for a Hopeful Smile

## 💡 Our Mission

Small choices create an impact, so keep it Sussy but Classy.

We aim to encourage conscious fashion choices by creating designs that value both style and sustainability. Every piece represents a step toward a more responsible and fashionable future.

## 🌱 Values

- ♻️ Sustainable Fashion
- 🎨 Creativity & Innovation
- 🌎 Environmental Responsibility
- ✨ Confidence Through Style

### The Design

The design focuses on combining sustainability with a bold and expressive visual identity. The vibrant blue and red color palette represents confidence, creativity, and passion while keeping the message of environmental responsibility clear. The handwritten typography and playful elements create a youthful yet classy feel that matches the brand’s personality.


---
<br>

### The Samal Sustainable Tourism Improvement System

#### 1. System Prompt Template (ChatGPT-Free)
"Act as a tourism advisor for Mindanao’s Samal Island tourism department. Your objective is to develop tourism plans and to promote the island, also keeping sustainability in mind. 

Context: The island if samal is a local tourist spot, but its popularity is not like the likes of boracay, siargao, and palawan.  
Constraints: The tone should feel formal, but not too stiff. It must also be based on local and national context. 
Format: develop 3 actionable plans for each senario, that are sustainable and environmentally friendly, number each from most effective, make the project title '### Samal Growth Tourism Action Plan'."

#### 2. Prompt Battle Ledger
| Version | Prompt Modifier Added | Output Quality Reflection |
| :--- | :--- | :--- |
| V1 | "Write a plan to prevent increased damage to aquatic life while promoting unknown beach spots in Samal." | Too generic. Did not specify spots |
| V2 | Added specific beach spots | Good. But it did not keep the semi formal light tone |
| V3 | Added a limitation to paragraphs and focus more on direct points for action | Pefrect. Actionable and can be reasonably acheived |

#### 3. Visual Branding Asset
- **Engine Used:** Gemini 3.5 Thinking
- **Visual Prompt:** "A minimalist vector logo for "Samal Island Sustainable Tourism Growth Plan". The design features a stylized fusion of a tropical leaf and a gentle ocean wave, seamlessly forming a subtle letter "S". Clean lines, flat design, modern, and professional. Color palette: Emerald green, deep teal, and a touch of warm sand gold. White background, corporate yet eco-friendly, high-end sustainable tourism branding. --no photo realistic shading --v 6.0 

<br>

### Literature Verification Log
#### Topic: Mindanao Growth and Development Through Infrastructure Investments 
#### 1. AI-Generated Summary Audit
I prompted an AI discovery tool to summarize recent literature on the Mindanao growth scene through infrastructure . Below is the verification tracking matrix:
| AI-Generated Statement / Citation | Source Vetted Against | Status | Human Correction / Empirical Note |
| :--- | :--- | :--- | :--- |
| "The government’s medium-term infrastructure program identified 1,310 infrastructure projects in Mindanao worth around ₱1.2 trillion, including major connectivity projects." | Department of Finance | **Approved** | The report validated that among the 3,770 infrastructure priority programs and projects, 1310 are focused on Mindanao, amounting to the said amount ₱1.2 trillion. 
| "Northern Mindanao cities have also shown strong growth, with Iligan City recording 8.8% economic growth in 2024, supported by expansion in services and local economic activity." | Philippines Information Agency  (PIA) |  **Approved** | Verified, Iligan City is shown to be one of the cities in Mindanao that has a strong economic growth in the article.
| "Nationally, infrastructure spending reached ₱1.545 trillion in 2024, equivalent to around 5.8% of GDP, showing continued government prioritization of infrastructure-led development." | Philippine News Agency |  **Good, but no camparison of data** | According to the data, from ₱1.419 trillion in 2023, it rose to ₱1.545 trillion, showing a continued prioritization by the government toward infrastructure led developments. The addition of this will provide more validity and comparison data.
#### 2. Critical Reflection on Tool Limitations 
“While the AI platform performed well and used valid data for its answers, the product was lacking in comparison of given data in the articles."

<br>

### Data Analytics & Visual Report

#### Dataset Focus: Mindanao Regional Environmental and Economic Surveillance (Mock CSV Analysis)

#### 1. Data Cleaning Protocol Log
- **Raw Input Problem:** The received CSV file contained inconsistent municipality and province naming conventions (e.g., "Davao city" vs. "DAVAO CITY"), missing values in energy output records, mixed verification labels, and irregular formatting in numerical fields.
- **AI Cleaning Instruction:**
`"Scan this dataset. Standardize all municipality and province names into a uniform format. Flag incomplete records, and normalize all numerical fields for comparison. Output the first 5 rows of the cleaned dataset."`
- **Result:** Successfully cleaned and standardized 45 municipal-level records covering agricultural production, renewable energy generation, and waste management indicators from 2021–2025. Missing entries were isolated for policy review rather than removed to preserve regional trends.

#### 2. Visualizations Generated

*(Embedded High-Contrast Line Chart showing Renewable Energy Output Growth Across Selected Mindanao Cities from 2021–2025)*

*(Embedded High-Contrast Bar Chart comparing Municipal Waste Generation Increase Between 2021 and 2025)*

#### 3. Human Analytical Narrative (The 'Why' Factor)

"The data shows that renewable energy production in major Mindanao cities increased from 2021–2025. However, waste generation also increased, showing that economic growth and urban development are creating more pressure on the environment.
The results suggest that the issue is not only about population growth but also about whether cities have enough systems to manage the effects of development. As cities grow, LGUs should invest in better waste management, recycling programs, and sustainable infrastructure to support growth while protecting the environment."


Thanks for visiting my profile! 😊
